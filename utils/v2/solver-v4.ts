...
export class GridModelV4 {
    ...
    public clearCellValue(index: number): void {
        this.cells[index] = 0;
        this.candidates[index] = ALL_CANDIDATES;
        this.recalculateSolved();
        this.updateCandidates();
    }

    private recalculateSolved(): void {
        this.rowSolved.fill(0);
        this.colSolved.fill(0);
        this.boxSolved.fill(0);
        for (let i = 0; i < 81; i++) {
            if (this.cells[i] !== 0) {
                const mask = 1 << (this.cells[i] - 1);
                const r = Math.floor(i / 9);
                const c = i % 9;
                const b = Math.floor(r / 3) * 3 + Math.floor(c / 3);
                this.rowSolved[r] |= mask;
                this.colSolved[c] |= mask;
                this.boxSolved[b] |= mask;
            }
        }
    }
    ...
    public updateCandidates(): void {
        for (let i = 0; i < 81; i++) {
            ...
            this.candidates[i] = ALL_CANDIDATES & ~solvedInPeers;
        }
    }
...
// 0. Full House (1.0)
export class FullHouseStrategy implements Strategy {
    find(grid: GridModelV4): StepAction | null {
        for (let h = 0; h < 27; h++) {
            const cells = getHouseCells(h);
            const empty = cells.filter(idx => grid.cells[idx] === 0);
            if (empty.length === 1) {
                const idx = empty[0];
                ...
                if (vals.length === 1) {
                    ...
                    return {
                        technique: "Last Digit",
                        difficulty: 1.0,
                        explanation: `The Last Digit in ${h < 9 ? 'Row' : h < 18 ? 'Column' : 'Block'} ${h % 9 + 1} is a ${v}.`,
                        row: r, col: c, val: v,
        ...
        for (let v = 1; v <= 9; v++) {
            ...
            if (count === 1) {
                ...
                const houseName = houseIdx < 9 ? `Row ${houseIdx + 1}` :
                    houseIdx < 18 ? `Column ${houseIdx - 9 + 1}` :
                        `Block ${houseIdx - 18 + 1}`;
                return {
                    technique,
                    difficulty,
                    explanation: `${technique}: In ${houseName}, the number ${v} can only fit in cell R${r + 1}C${c + 1}.`,
                    row: r, col: c, val: v,
                    highlights: [{ row: r, col: c, type: 'target', val: v }]
                };
            }
                    ...
                    if (rows.size === 1) {
                        ...
                        if (elims.length > 0) {
                            return {
                                ...
                                difficulty: 2.6,
                                explanation: `Pointing Pair: Since ${v} in Block ${b + 1} must be in Row ${r + 1}, it cannot be elsewhere in the row`,
                                val: v,
                                highlights: [
                    ...
                    if (cols.size === 1) {
                        ...
                        if (elims.length > 0) {
                            return {
                                technique: "Pointing Pair",
                                difficulty: 2.6,
                                explanation: `Pointing Pair: Since ${v} in Block ${b + 1} must be in Column ${c + 1}, it cannot be elsewhere in the column`,
                                val: v,
                                highlights: [
                                    ...positions.map(p => ({ row: Math.floor(p / 9), col: p % 9, type: 'hint' as const })),
                                    ...elims.map(idx => ({ row: Math.floor(idx / 9), col: idx % 9, type: 'elimination' as const, val: v }))
                    ...
                    if (boxes.size === 1) {
                        ...
                        if (elims.length > 0) {
                            return {
                                ...
                                difficulty: 2.8,
                                explanation: `Claiming Pair: Since ${v} in ${h < 9 ? 'Row' : 'Column'} ${h % 9 + 1} is restricted to Block ${b + 1}, it cannot appear elsewhere in the block`,
                                val: v,
                                highlights: [
                                    ...positions.map(p => ({ row: Math.floor(p / 9), col: p % 9, type: 'hint' as const })),
                ...
                if (cellsWithDigits.length === this.n) {
                    ...
                    if (elims.length > 0) {
                        return {
                            ...
                            difficulty,
                            explanation: `${names[this.n]}: The numbers ${comboDigits.join(',')} are hidden in ${h < 9 ? 'Row' : h < 18 ? 'Column' : 'Block'} ${h % 9 + 1}`,
                            highlights: [
        ...
        for (let h = 0; h < 27; h++) {
            ...
            for (const combo of combinations) {
                ...
                if (GridModelV4.countSetBits(mask) === this.n) {
                    ...
                    // Identify ALL houses shared by ALL cells in the combo
                    const sharedHouses: number[] = [];
                    const r0 = Math.floor(combo[0] / 9);
                    const c0 = combo[0] % 9;
                    const b0 = Math.floor(r0 / 3) * 3 + Math.floor(c0 / 3);
                    if (combo.every(idx => Math.floor(idx / 9) === r0)) sharedHouses.push(r0);
                    if (combo.every(idx => (idx % 9) === c0)) sharedHouses.push(c0 + 9);
                    if (combo.every(idx => (Math.floor(Math.floor(idx / 9) / 3) * 3 + Math.floor((idx % 9) / 3)) === b0)) sharedHouses.push(b0 + 18);

                    const elims: number[] = [];
                    for (const sh of sharedHouses) {
                        for (const idx of HOUSES[sh]) {
                            if (grid.cells[idx] === 0 && !combo.some(ci => ci === idx) && (grid.candidates[idx] & mask)) {
                                if (!elims.includes(idx)) elims.push(idx);
                            }
                        }
                    }
                    ...
                    if (elims.length > 0) {
                        ...
                        return {
                            technique: names[this.n],
                            difficulty,
                            explanation: `${names[this.n]}: The candidates ${GridModelV4.getVals(mask).join(',')} are locked in ${h < 9 ? 'Row' : h < 18 ? 'Column' : 'Block'} ${h % 9 + 1}`,
                            highlights
                        };
                    }
                }
...
export class FishStrategy implements Strategy {
    ...
    private findFishInUnits(grid: GridModelV4, v: number, vMask: number, startH: number, endH: number, difficulty: number, technique: string): StepAction | null {
        const potentialBaseUnits: { h: number, mask: number, cells: number[] }[] = [];
        for (let h = startH; h < endH; h++) {
            let mask = 0;
            const cells = [];
            for (const idx of HOUSES[h]) {
                if (grid.candidates[idx] & vMask) {
                    const pos = (startH === 0) ? (idx % 9) : Math.floor(idx / 9);
                    mask |= (1 << pos);
                    cells.push(idx);
                }
            }
            if (cells.length >= 2) {
                potentialBaseUnits.push({ h, mask, cells });
            }
        }

        if (potentialBaseUnits.length < this.n) return null;

        const combos = getCombinations(potentialBaseUnits, this.n);
        for (const combo of combos) {
            let columnUnionMask = 0;
            for (const unit of combo) columnUnionMask |= unit.mask;

            if (GridModelV4.countSetBits(columnUnionMask) === this.n) {
                const fishCells = combo.flatMap(u => u.cells);
                const elims: { row: number; col: number; type: 'elimination'; val: number }[] = [];

                for (let pos = 0; pos < 9; pos++) {
                    if (columnUnionMask & (1 << pos)) {
                        const coverHouseIdx = (startH === 0) ? (pos + 9) : pos;
                        for (const idx of HOUSES[coverHouseIdx]) {
                            if (!fishCells.includes(idx) && (grid.candidates[idx] & vMask)) {
                                elims.push({
                ...
                if (elims.length > 0) {
                    return {
                        technique,
                        difficulty,
                        explanation: `${technique}: The digit ${v} forms a pattern in ${startH === 0 ? 'Rows' : 'Columns'}`,
                        val: v,
                        highlights: [
                            ...fishCells.map(idx => ({ row: Math.floor(idx / 9), col: idx % 9, type: 'hint' as const })),
                            ...elims
                        ]
                    };
                }
                        ...
                        if (C1 === C2) {
                            ...
                            if (elims.length > 0) {
                                return {
                                    technique: "XY-Wing",
                                    difficulty: 4.2,
                                    explanation: `XY-Wing: A pivot cell at R${Math.floor(pivot / 9) + 1}C${pivot % 9 + 1} connects two pincer cells, forcing ${elimDigit} to be eliminated`,
                                    val: elimDigit,
                                    highlights: [
                    ...
                    if (A && B && A !== B && pivotVals.includes(A) && pivotVals.includes(B)) {
                        ...
                        if (elims.length > 0) {
                            return {
                                technique: "XYZ-Wing",
                                difficulty: 4.4,
                                explanation: `XYZ-Wing: A pivot cell at R${Math.floor(pivot / 9) + 1}C${pivot % 9 + 1} connects pincers, forcing ${C} to be eliminated`,
                                val: C,
                                highlights: [
                                    { row: Math.floor(pivot / 9), col: pivot % 9, type: 'hint' },
                                    { row: Math.floor(p1 / 9), col: p1 % 9, type: 'hint' },
...
export class UniqueRectangleStrategy implements Strategy {
    find(grid: GridModelV4): StepAction | null {
        // Find all potential Floor bivalue cells
        // Optimization: Pre-calculate candidate counts
        const bivalueCells: number[] = [];
        for (let i = 0; i < 81; i++) {
            if (grid.cells[i] === 0 && GridModelV4.countSetBits(grid.candidates[i]) === 2) {
                bivalueCells.push(i);
        ...
        // Iterate pairs of bivalue cells that could be a Floor
        // A Floor must be in the same Row, Col, or Block.
        // For standard UR, we usually look for Row-aligned or Col-aligned floors.
        // Let's iterate Row Floors: (r, c1) and (r, c2).

        // Group bivalue cells by Row
        const rows = new Map<number, number[]>();
        for (const idx of bivalueCells) {
            const r = Math.floor(idx / 9);
            if (!rows.has(r)) rows.set(r, []);
            rows.get(r)!.push(idx);
        }

        for (const [r1, cells] of rows) {
            if (cells.length < 2) continue;
            // Check pairs
            const combos = getCombinations(cells, 2);
            for (const [c1, c2] of combos) {
                // Potential Floor: (r1, c1) and (r1, c2).
                if (grid.candidates[c1] !== grid.candidates[c2]) continue; // Must be same pair {A,B}

                const mask = grid.candidates[c1];
                const parts = GridModelV4.getVals(mask);
                const v1 = parts[0];
                const v2 = parts[1];
                ...
                // Check Geometry: Must be in same block? 
                // Standard UR typically has Floor in one block, Roof in another.
                // But generally, the 4 cells just need to be in exactly 2 blocks (parallel).
                const b1 = Math.floor(r1 / 3) * 3 + Math.floor(col1 / 3);
                const b2 = Math.floor(r1 / 3) * 3 + Math.floor(col2 / 3);

                // If Floor is in 2 blocks, Roof must be in same 2 blocks?
                // Standard UR: 2 cells in Block A, 2 cells in Block B.
                // If b1 !== b2, then Floor spans 2 blocks. Then Roof must span same 2 blocks.
                // If b1 === b2, then Floor is in 1 block. Roof must be in 1 block (different one).

                // Iterate all other rows for Roof
                for (let r2 = 0; r2 < 9; r2++) {
                    if (r1 === r2) continue;

                    const c3 = r2 * 9 + col1; // Roof 1
                    const c4 = r2 * 9 + col2; // Roof 2

                    // Validate Roof cells exist and are unsolved
                    if (grid.cells[c3] !== 0 || grid.cells[c4] !== 0) continue;

                    // Check Block Geometry
                    const b3 = Math.floor(r2 / 3) * 3 + Math.floor(col1 / 3); // should be same block col as b1
                    const b4 = Math.floor(r2 / 3) * 3 + Math.floor(col2 / 3);

                    // The 4 cells (c1, c2, c3, c4) must typically fall into exactly 2 blocks.
                    // Case A: Floor in Bx, Roof in By. (b1==b2, b3==b4, b1!=b3, b1,b3 in same band).
                    // Case B: Left side in Bx, Right side in By. (b1==b3, b2==b4, b1!=b2).
                    // If neither, skip.
                    let validGeo = false;
                    if (b1 === b2 && b3 === b4 && b1 !== b3) validGeo = true; // Horizontal parallel blocks
                    if (b1 === b3 && b2 === b4 && b1 !== b2) validGeo = true; // Vertical parallel blocks

                    if (!validGeo) continue;

                    // Candidates check
                    const m3 = grid.candidates[c3];
                    const m4 = grid.candidates[c4];

                    // Both roof cells must contain the pair (mask)
                    if ((m3 & mask) !== mask || (m4 & mask) !== mask) continue;

                    // Analyze Extra Candidates
                    const extra3 = m3 & ~mask;
                    const extra4 = m4 & ~mask;

                    // TYPE 1: One roof cell has NO extra (so it is {A,B}), the other has extra.
                    // Note: If both have no extra, it's a Deadly Pattern (invalid puzzle or we made mistake).
                    if (extra3 === 0 && extra4 !== 0) {
                        // Elim {A,B} from c4? No.
                        // If c1,c2,c3 are {A,B}, then c4 CANNOT be A or B. 
                        // Because if c4 is A, c3 is B... 2 sol loop.
                        // So c4 MUST be (extra).
                        // Eliminate A and B from c4.
                        return {
                            technique: "Unique Rectangle (Type 1)",
                            difficulty: 4.5,
                            explanation: `Unique Rectangle (Type 1): Cell R${r2 + 1}C${col2 + 1} cannot be ${v1} or ${v2} to avoid a deadly pattern`,
                            highlights: [
                                { row: r1, col: col1, type: 'hint' }, { row: r1, col: col2, type: 'hint' }, { row: r2, col: col1, type: 'hint' },
                                { row: r2, col: col2, type: 'elimination', val: v1 },
                                { row: r2, col: col2, type: 'elimination', val: v2 }
                            ]
                        };
                    }
                    if (extra4 === 0 && extra3 !== 0) {
                        return {
                            technique: "Unique Rectangle (Type 1)",
                            difficulty: 4.5,
                            explanation: `Unique Rectangle (Type 1): Cell R${r2 + 1}C${col1 + 1} must take the extra candidates to avoid a deadly pattern`,
                            highlights: [
                                { row: r1, col: col1, type: 'hint' }, { row: r1, col: col2, type: 'hint' }, { row: r2, col: col2, type: 'hint' },
                                { row: r2, col: col1, type: 'elimination', val: v1 },
                                { row: r2, col: col1, type: 'elimination', val: v2 }
                            ]
                        };
                    }
                    ...
                    if (extra3 !== 0 && extra4 !== 0) {
                        ...
                        // TYPE 2: Extras are identical? (e.g. both have 'X').
                        if (extra3 === extra4) {
                            // One of c3/c4 MUST be X.
                            // So X can be eliminated from any cell that sees BOTH c3 and c4.
                            // Shared units: Row r2. Blocks? (If b3==b4). Cols? No.
                            const extraVals = GridModelV4.getVals(extra3);
                            if (extraVals.length === 1) { // Only do simplified Type 2 for single extra
                                const x = extraVals[0];
                                const elims: any[] = [];

                                // Scan Row r2
                                HOUSES[r2].forEach(idx => {
                                    if (idx !== c3 && idx !== c4 && (grid.candidates[idx] & (1 << (x - 1)))) {
                                        elims.push({ row: Math.floor(idx / 9), col: idx % 9, type: 'elimination', val: x });
                                    }
                                });
                                // Scan Block if shared
                                if (b3 === b4) {
                                    HOUSES[b3 + 18].forEach(idx => {
                                        if (idx !== c3 && idx !== c4 && (grid.candidates[idx] & (1 << (x - 1)))) {
                                            // Dedupe done by StepAction consumer usually? Or verify manually.
                                            // Just push, usually fine or check logic.
                                            // Check strict equality to avoid dupes logic here is annoying.
                                            // Let's iterate unique indices.
                                            // ... skipping complexity for now, Row 2 is primary.
                                        }
                                    });
                                }
                                ...
                                if (elims.length > 0) {
                                    return {
                                        ...
                                        difficulty: 4.5,
                                        explanation: `Unique Rectangle (Type 2): One of the roof cells must be ${x} to avoid a deadly pattern`,
                                        highlights: [
                                            { row: r1, col: col1, type: 'hint' }, { row: r1, col: col2, type: 'hint' },
                                            { row: r2, col: col1, type: 'hint' }, { row: r2, col: col2, type: 'hint' },
                                            ...elims
                                        ]
                                    };
                                }
                        ...
                        // TYPE 4: Locked Candidate
                        // Check if v1 or v2 is locked in Roof (in Row r2, Col col1/col2? No, usually check the Roof House).
                        // Since we iterated Rows, Roof House is Row r2.
                        // Check if v1 exists anywhere else in Row r2.
                        let v1Locked = true;
                        let v2Locked = true;

                        for (const idx of HOUSES[r2]) {
                            if (idx === c3 || idx === c4) continue;
                            if (grid.candidates[idx] & (1 << (v1 - 1))) v1Locked = false;
                            if (grid.candidates[idx] & (1 << (v2 - 1))) v2Locked = false;
                        }

                        if (v1Locked && !v2Locked) {
                            // v1 is locked in c3/c4. So v1 MUST be in one of them.
                            // To avoid deadly pattern (v1/v1), we can't have v2/v2?
                            // Logic: If we put v2 in c3 and v2 in c4? No, impossible (row distinct).
                            // Logic: If v1 is locked, then 'v2' can be eliminated from c3 and c4.
                            // If we allow v2, we might force v1 out?
                            return {
                                technique: "Unique Rectangle (Type 4)",
                                difficulty: 4.5,
                                explanation: `Unique Rectangle (Type 4): The digit ${v1} is locked in the roof cells, forcing ${v2} to be eliminated`,
                                highlights: [
                                    { row: r1, col: col1, type: 'hint' }, { row: r1, col: col2, type: 'hint' },
                                    { row: r2, col: col1, type: 'elimination', val: v2 },
                                    { row: r2, col: col2, type: 'elimination', val: v2 }
                        ...
                        if (v2Locked && !v1Locked) {
                            return {
                                technique: "Unique Rectangle (Type 4)",
                                difficulty: 4.5,
                                explanation: `Unique Rectangle (Type 4): The digit ${v2} is locked in the roof cells, forcing ${v1} to be eliminated`,
                                highlights: [
                                    { row: r1, col: col1, type: 'hint' }, { row: r1, col: col2, type: 'hint' },
                                    { row: r2, col: col1, type: 'elimination', val: v1 },
                                    { row: r2, col: col2, type: 'elimination', val: v1 }
        ...
        // Col-based Floor Logic? (Skipped for now, usually Rows cover it via symmetry or just rotation... 
        // wait. If the UR is vertical, "Group by Row" finds (r1,c1)-(r1,c2) which IS A ROW FLOOR.
        // If UR is horizontal (Floor in Col 1, Col 2?), then "Group by Row" finds "Roof" in Row...
        // Essentially, iterating Row Floors covers all orthogonal rectangles.)
            ...
            for (const v of vals) {
                ...
                if (rCount === 3 && cCount === 3 && bCount === 3) {
                    return {
                        ...
                        difficulty: 5.6,
                        explanation: `BUG Type 1: Cell R${r + 1}C${c + 1} must be ${v} to avoid an invalid state`,
                        row: r, col: c, val: v,
                        highlights: [{ row: r, col: c, type: 'target', val: v }]
                    };
                }
        ...
        for (let i = 0; i < 81; i++) {
            ...
            for (const v of vals) {
                ...
                if (elims[0] === -1) {
                    return {
                        ...
                        difficulty: 7.5 + getChainLengthPenalty(elims.length * 2), // Approx nodes
            ...
            if (commonElims.length > 0) {
                return {
                    ...
                    difficulty: 8.2 + getChainLengthPenalty(commonElims.length * vals.length * 2),
            ...
            for (const v of vals) {
                ...
                if (SudokuSolverV4.countSolutions(nextGrid.cells.join('').split('').map(Number)) === 0) {
                    return {
                        ...
                        difficulty: 8.5 + getChainLengthPenalty(100), // Dynamic (recursive) gets heavy penalty
        ...
        for (let i = 0; i < 81; i++) {
            ...
            for (const v of vals) {
                ...
                if (elims[0] === -1) {
                    return {
                        technique: "Contradiction Forcing Chain",
                        difficulty: 7.5 + getChainLengthPenalty(elims.length * 2), // Spec: Nishio Base
                        explanation: `Contradiction (Propagated): Cell (${Math.floor(i / 9) + 1},${i % 9 + 1}) cannot be ${v}`,
                        highlights: [
                            { row: Math.floor(i / 9), col: i % 9, type: 'hint' },
                            { row: Math.floor(i / 9), col: i % 9, type: 'elimination', val: v }
            ...
            for (const v of vals) {
                ...
                if (SudokuSolverV4.countSolutions(nextGrid.cells.join('').split('').map(Number)) === 0) {
                    return {
                        technique: "Contradiction Forcing Chain (Deep)",
                        difficulty: 8.5 + getChainLengthPenalty(100), // Base 8.5 for Dynamic
                        explanation: `Contradiction (Deep): Cell (${Math.floor(i / 9) + 1},${i % 9 + 1}) cannot be ${v}`,
                        highlights: [
                            { row: Math.floor(i / 9), col: i % 9, type: 'hint' },
                            { row: Math.floor(i / 9), col: i % 9, type: 'elimination', val: v }
...
export class AICStrategy implements Strategy {
    find(grid: GridModelV4): StepAction | null {
        ...
        for (let limit = 4; limit <= 30; limit += 2) {
        ...
        if (!nextIsStrong && path.length >= 4) {
            ...
            if (start.v === end.v && (start.r !== end.r || start.c !== end.c)) {
                ...
                if (elims.length > 0) {
                    ...
                    const isXChain = path.every(n => n.v === start.v);
                    const baseScore = isXChain ? 6.6 : 7.0;
                    const technique = isXChain ? "Forcing X-Chain" : "Forcing Chain";
                    const difficulty = baseScore + penalty;
                    return {
                        technique,
                    ...
                    if (r1 !== r2 && col1 !== col2) {
                        ...
                        if (rowCells.length === 2 && colCells.length === 2) {
                            ...
                            if (rowEndIdx !== undefined && colEndIdx !== undefined) {
                                const elimIdx = Math.floor(colEndIdx / 9) * 9 + (rowEndIdx % 9);
...
export class WXYZWingStrategy implements Strategy {
    find(grid: GridModelV4): StepAction | null {
        // Extended WXYZ Wing logic
        for (let p = 0; p < 81; p++) {
            if (grid.cells[p] !== 0) continue;
            const pCands = GridModelV4.getVals(grid.candidates[p]);
            if (pCands.length < 2 || pCands.length > 4) continue;

            const pPeers = new Set(PEERS[p]);

            const potentialWings = [];
            for (let i = 0; i < 81; i++) {
                if (i === p) continue;
                if (grid.cells[i] !== 0) continue;
                const iCands = GridModelV4.getVals(grid.candidates[i]);
                if (iCands.length < 2 || iCands.length > 4) continue;

                if ((grid.candidates[i] & grid.candidates[p]) === 0) continue;
                if (pPeers.has(i)) {
                    potentialWings.push(i);
                }
            }

            if (potentialWings.length < 3) continue;

            const combos = getCombinations(potentialWings, 3);
            for (const wings of combos) {
                const [w1, w2, w3] = wings;

                let mask = grid.candidates[p] | grid.candidates[w1] | grid.candidates[w2] | grid.candidates[w3];
                if (GridModelV4.countSetBits(mask) !== 4) continue;

                const allVals = GridModelV4.getVals(mask);

                for (const z of allVals) {
                    const mNoZ = mask & ~(1 << (z - 1));

                    if (GridModelV4.countSetBits(mNoZ) <= 3) {
                        const zCells = [p, w1, w2, w3].filter(idx => (grid.candidates[idx] & (1 << (z - 1))));
                        if (zCells.length === 0) continue;

                        const firstPeers = PEERS[zCells[0]];
                        const commonPeers = firstPeers.filter(peerIdx => {
                            for (let k = 1; k < zCells.length; k++) {
                                if (!PEERS[zCells[k]].includes(peerIdx)) return false;
                            }
                            return true;
                        });

                        const elims = [];
                        for (const t of commonPeers) {
                            if ([p, w1, w2, w3].includes(t)) continue;
                            if (grid.cells[t] !== 0) continue;
                            if (grid.candidates[t] & (1 << (z - 1))) {
                                elims.push({ row: Math.floor(t / 9), col: t % 9, type: 'elimination' as const, val: z });
                            }
                        }
                        ...
                        if (elims.length > 0) {
                            return {
                                technique: "WXYZ-Wing",
                                difficulty: 5.5,
                                explanation: `WXYZ-Wing on ${allVals.join(',')} forces ${z}`,
                                highlights: [
                                    { row: Math.floor(p / 9), col: p % 9, type: 'hint' },
                                    { row: Math.floor(w1 / 9), col: w1 % 9, type: 'hint' },
                                    { row: Math.floor(w2 / 9), col: w2 % 9, type: 'hint' },
                                    { row: Math.floor(w3 / 9), col: w3 % 9, type: 'hint' },
                                    ...elims
                                ]
                            };
                        }
...
export function sees(i1: number, i2: number): boolean {
    return PEERS[i1].includes(i2);
}

export function getCommonHouse(i1: number, i2: number): number {
...
export function getCommonPeers(i1: number, i2: number): number[] {
    const p1 = PEERS[i1];
    const p2 = PEERS[i2];
        ...
        for (let h = 0; h < 27; h++) {
            ...
            for (const comboDigits of combinations) {
                ...
                if (cellsWithDigits.length === n) {
                    ...
                    // Check Logic: Verify no other cell in the house contains A or B
                    for (const idx of emptyCells) {
                    ...
                    // CHECK: Does this enable a Hidden Single in ANY intersecting house?
                    ...
                    // Houses to check: The current house 'h' is usually enough for standard Hidden Pair, 
                    // but "Direct" implies we might affect crossing houses.
                    // We need to check any house that contains a cell from which we eliminated a candidate.
                    // Since eliminations happen in 'cellsWithDigits', we check all houses passing through 'cellsWithDigits'.

                    const housesToCheck = new Set<number>();
                    housesToCheck.add(h); // Always check origin house

                    for (const idx of cellsWithDigits) {
                        const r = Math.floor(idx / 9);
                        const c = idx % 9;
                        const b = Math.floor(r / 3) * 3 + Math.floor(c / 3);
                        housesToCheck.add(r);      // Row (0-8)
                        housesToCheck.add(9 + c);  // Col (9-17)
                        housesToCheck.add(18 + b); // Block (18-26)
                    }

                    for (const v of eliminatedValues) {
                        for (const houseIdx of housesToCheck) {
                            let count = 0;
                            let lastIdx = -1;
                            const hCells = HOUSES[houseIdx];

                            for (const idx of hCells) {
                                if (grid.cells[idx] !== 0) continue;

                                let hasVal = (grid.candidates[idx] & (1 << (v - 1))) !== 0;

                                // Simulate Elimination: If idx is one of the pair cells, 'v' is removed (since v is in eliminatedValues)
                                if (cellsWithDigits.includes(idx)) hasVal = false;
                    ...
                    // CHECK 2: Does this enable a NAKED Single in any affected cell?
                    // (Affected cells are those where eliminations occurred)
                    if (!foundDirect) {
                        const cellElims = new Map<number, number>(); // idx -> mask of eliminated vals

                        // Standard Elims (from Pair Cells)
                        for (const e of elims) {
                            const eIdx = e.row * 9 + e.col;
                            const existing = cellElims.get(eIdx) || 0;
                            cellElims.set(eIdx, existing | (1 << (e.val - 1)));
                        }

                        // CHECK 3 (Critical for P71): Intersection Eliminations (Pointing Hidden Pair)
                        // If the Hidden Pair cells are all in the same Block (and we found them in Row/Col),
                        // we can eliminate the Pair Candidates from the rest of the Block.
                        if (h < 18) { // Row or Col
                            const blockIndices = new Set(cellsWithDigits.map(c => {
                                const r = Math.floor(c / 9);
                                const cIdx = c % 9;
                                return Math.floor(r / 3) * 3 + Math.floor(cIdx / 3);
                            }));

                            if (blockIndices.size === 1) {
                                // Confined to one block!
                                const bIdx = Array.from(blockIndices)[0];
                                const blockCells = HOUSES[18 + bIdx]; // Blocks are 18-26

                                // Elim pair values (mask) from other cells in Block
                                for (const bCell of blockCells) {
                                    if (grid.cells[bCell] !== 0) continue;
                                    if (cellsWithDigits.includes(bCell)) continue;

                                    // Check if bCell has any of the pair values
                                    if ((grid.candidates[bCell] & mask) !== 0) {
                                        const lostVals = grid.candidates[bCell] & mask;
                                        const existing = cellElims.get(bCell) || 0;
                                        cellElims.set(bCell, existing | lostVals);
                                    }
                                }
                            }
                        } else {
                            // Case B: Found in Block, check confinement to Row or Col (Claiming)
                            // Row Check
                            const rows = new Set(cellsWithDigits.map(c => Math.floor(c / 9)));
                            if (rows.size === 1) {
                                const r = Array.from(rows)[0];
                                const rowCells = HOUSES[r];
                                for (const rCell of rowCells) {
                                    if (grid.cells[rCell] !== 0) continue;
                                    if (cellsWithDigits.includes(rCell)) continue;
                                    if ((grid.candidates[rCell] & mask) !== 0) {
                                        const lostVals = grid.candidates[rCell] & mask;
                                        const existing = cellElims.get(rCell) || 0;
                                        cellElims.set(rCell, existing | lostVals);
                                    }
                                }
                            }
                            // Col Check
                            const cols = new Set(cellsWithDigits.map(c => c % 9));
                            if (cols.size === 1) {
                                const c = Array.from(cols)[0];
                                const colCells = HOUSES[9 + c];
                                for (const cCell of colCells) {
                                    if (grid.cells[cCell] !== 0) continue;
                                    if (cellsWithDigits.includes(cCell)) continue;
                                    if ((grid.candidates[cCell] & mask) !== 0) {
                                        const lostVals = grid.candidates[cCell] & mask;
                                        const existing = cellElims.get(cCell) || 0;
                                        cellElims.set(cCell, existing | lostVals);
                                    }
                                }
                            }
                        }

                        for (const [idx, removedMask] of cellElims.entries()) {
                            const current = grid.candidates[idx];
                            const nextCands = current & ~removedMask;
                            if (GridModelV4.countSetBits(nextCands) === 1) {
                                foundDirect = true;
                                const val = GridModelV4.getVals(nextCands)[0];
                                winningVal = val;
                                winningIdx = idx;

                                // Update elims to reflect the full set causing the win
                                elims.length = 0;
                                for (const [eIdx, eMask] of cellElims.entries()) {
                                    const r = Math.floor(eIdx / 9);
                                    const c = eIdx % 9;
                                    const vals = GridModelV4.getVals(eMask);
                                    for (const v of vals) elims.push({ row: r, col: c, type: 'elimination', val: v });
                                }
                                break;
                            }
                        }
...
export function gradePuzzleV4(puzzle: string | number[][]): { rating: number; steps: StepAction[]; solved: boolean } {
    ...
    const strategies: Strategy[] = [
        new FullHouseStrategy(),         // 1.0
        new HiddenSingleStrategy(),      // 1.2, 1.5
        new DirectHiddenPairStrategy(),  // 2.0
        new NakedSingleStrategy(),        // 2.3
        new PointingStrategy(),           // 2.6
        new ClaimingStrategy(),           // 2.8
